...
io = remote("node1.anna.nssctf.cn",24296)
...
v = int(io.recvline().strip())

resend(b'g')
io.sendlineafter(b'seed: \n',b'1')
io.recvuntil(b'_k) = ')

u = int(io.recvline().strip())
u = (u - v) % _n

def recvsign(msg):
    resend(b's')
    io.recvuntil(b'Please send your message: \n')
    io.sendline(msg)
    io.recvuntil(b'r = ')
...
r1,s1 = recvsign(b'a')
r2,s2 = recvsign(b'a')
h = bytes_to_long(sha512(b'a').digest())
