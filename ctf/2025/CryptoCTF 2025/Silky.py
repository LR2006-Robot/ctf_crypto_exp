...
p = remote("node1.anna.nssctf.cn",25372)
...
for i in range(10):
    ...
    for i in range(lr // 16):
        recv = p.recvline().strip().decode().replace(' ',',')[2:]
        R += literal_eval(recv)
...
def randroad(B):
	return vector(ZZ,[randint(-B, B) for _ in range(n)])	# 19个-5到5
